 ...
 *      Author: Sean
 *
 * Modified from: recursiveGOL.cpp by Sean and speculativeGOL.cpp by Thomas
 *
 * Compile as: g++ -fopenmp recSpecHybrid.cpp Board.cpp Cell.cpp -o rGOL
 */
...
void divideBoard(Board &board, int start, int end, int partitions) {
    ...
    if (end - start < partitions) {
        // base case, switch to speculative algorithm
        specAlg(board, start, end);
    } else {
        int p = partitions;
        bool done = false;
        do {
...
void specAlg(Board & board, int start, int end) {
